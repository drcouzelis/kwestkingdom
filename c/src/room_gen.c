...
FLAG is_path_between_points_recursively(PATH_ALG_DATA *data, int r1, int c1, int r2, int c2)
{
  /**
   * Warning!
   * Recursion! <3
  ...
  for (dir = NORTH; dir <= WEST; dir++) {
    ...
    if (data->mask[row][col] == ON && data->hist[row][col] == OFF) {
      if (is_path_between_points_recursively(data, row, col, r2, c2)) {
        return ON;
      }
    }
  }
...
FLAG is_path_between_points(PATH_ALG_DATA *data, int r1, int c1, int r2, int c2)
{
  ...
  */
  ...
  return is_path_between_points_recursively(data, r1, c1, r2, c2);
}
...
void change_room_theme(ROOM *room, ROOM_THEME theme)
{
  int i;
  ...
  add_frame(room->terrain_anims[TILE_TYPE_EMPTY], grab_image(empty, NORMAL));
  add_frame(room->terrain_anims[TILE_TYPE_HOLE], grab_image(hole, NORMAL));
  add_frame(room->terrain_anims[TILE_TYPE_WALL], grab_image(wall, NORMAL));
  add_frame(room->terrain_anims[TILE_TYPE_PATH], grab_image(path, NORMAL));
  add_frame(room->terrain_anims[TILE_TYPE_N_EDGE], grab_image(edge, NORMAL));
  add_frame(room->terrain_anims[TILE_TYPE_S_EDGE], grab_image(edge, V_FLIP));
  add_frame(room->terrain_anims[TILE_TYPE_E_EDGE], grab_image(edge, ROTATE));
  add_frame(room->terrain_anims[TILE_TYPE_W_EDGE], grab_image(edge, ROTATE_V_FLIP));
  add_frame(room->terrain_anims[TILE_TYPE_NE_INSIDE], grab_image(inside, NORMAL));
  add_frame(room->terrain_anims[TILE_TYPE_NW_INSIDE], grab_image(inside, H_FLIP));
  add_frame(room->terrain_anims[TILE_TYPE_SE_INSIDE], grab_image(inside, V_FLIP));
  add_frame(room->terrain_anims[TILE_TYPE_SW_INSIDE], grab_image(inside, H_V_FLIP));
  
  add_frame(room->terrain_anims[TILE_TYPE_NE_OUTSIDE], grab_image(outside, NORMAL));
  add_frame(room->terrain_anims[TILE_TYPE_NW_OUTSIDE], grab_image(outside, H_FLIP));
  add_frame(room->terrain_anims[TILE_TYPE_SE_OUTSIDE], grab_image(outside, V_FLIP));
  add_frame(room->terrain_anims[TILE_TYPE_SW_OUTSIDE], grab_image(outside, H_V_FLIP));
